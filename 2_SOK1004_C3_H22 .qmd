...
Ligningen viser prosentvis endring i KPI eller inflasjon i fra en bestemt måned og ett start år.
...
df3 <- df2 %>%
  group_by(dato)%>%
  summarize(df3 <- sum(vekt)) %>%
  na.omit(df3)
df3 %>% 
  summary()


#Største avviket er på 0,0514 og minste avviket er på 0,0233.
```
...
df4 %>%
  ggplot(aes(x = dato, y = vekt)) +
  geom_line() +
  labs(x = "Årstall", y = "KPI", title = "KPI til elektrisitet", subtitle = "KPI til elektrisitet standard år 2015.") +
  theme_bw()
```

*\[Besvar oppgave IV her.\]*

KPI er på y aksen, og årstall på x aksen. 2015 er standard året da KPI var på 100.

Grafen viser KPI endringen til elektrisitet fra 2011-2022.

Den billigste strømmen var i 2012 og 2013, etter dette har strømprisene økt helt frem til 2020-2021 hvor det falt kraftig. Etter dette fallet har strømprisene økt til det dyreste det har vært noensinne(2022).
...
Høye kraftpriser gjør det dyrere for bedrifter å produsere forskjellige tjenester/produkter dermed må de øke prisene på tjenestene/produktene. Samtidig blir det dyrere strøm til privatpersoner, for å kompensere for dette må lønnen øke for privatpersoner slik at man fortsatt kan ha vanlig forbruk. Dette gjør at det blir mer penger i sirkulasjon og dette er med på å øke inflasjon.
