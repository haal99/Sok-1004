...
df1 <- df[[2]]
names(df1) <- c("var", "tid", "verdi")
df1$tid <- as.integer(df1$tid)
...
df1%>%
  filter(var == "BNP" | var == "MEMOBNP") %>%
  filter(tid >= 2000) %>%
  ggplot(aes(x=tid,y=verdi/100, color=var)) +
  geom_line() +
  labs(title = "BNP and MEMOBNP from 2000 to 2021",
       x = "Year",
       y = "Value",
       color = "Variable")
...
Ut i fra det vi kan se på grafen kan vi se at BNP per innbygger har mer enn doblet seg fra 2000-2021.
BNP i løpende priser tar utgangspunkt i de faktiske prisene det gjeldende året, mens faste priser tar høyde for inflasjon og endret pengeverdi.
Faste priser er mest relevant for å se produksonutvikling og brukes for å se utviklingen over tid, mens løpende priser er mest relevant for å se den den faktiske BNPén til ett land.
...
Koden utvidet dataene, økte antall kolonner og reduserer antall rader.
...
# Besvar oppgave IIc 
df_long <- df_wide %>%
  pivot_longer(cols = c(DBNP,DMEMOBNP), names_to = "var", values_to = "verdi")
```
...
## Besvarelse på oppgave IId:

Vi ser at det finnes flere observasjoner med negativ vekst i reel BNP, det er nærliggende å tro at negativ vekst i 2020 kan skyldes covid og lavere produksjon av forskjellige tjenester. 
...
ggplot(df_long, aes(x = tid, y = verdi, color = var)) +
  geom_point() +
  geom_smooth() +
  labs(title = "Vekst i BNP per person",
       x = "År",
       y = "Vekst i BNP per person",
       color = "Vekst") +
  theme_minimal()
```
