...
#Når du har bindwidth på 0.1 så får du med alle datapunktene men det er vanskelig å lese av de. Samme hvis du øker bindwidth til eks 3, så minsker antall data og du får brede stolper som gjør det vanskelig å lese av data. Derfor er det viktig å finne en bindwidth som gjør at dataene blir presentert på best mulig måte for å kunne lese de av. 
```
...
df <- df %>%
mutate(Verdi_milliarder=value/1000)
...
df2 %>%
 ggplot(aes(x = Verdi_milliarder, fill = sysselsettingsgruppe)) +
...
df3 %>%
  ggplot(aes(x = Verdi_milliarder, fill = statistikkvariabel)) +
  geom_histogram(color = "black", binwidth = 0.8) +
  facet_wrap(~ næring, scale = "free_y") +
  theme_bw()
